...
mission "translateGameTypeEarnMoney"{
	...
	state Initialize{
		...
		if(comboMoneyToEarn==0){
			nMoneyToEarn=25000;
		}else if(comboMoneyToEarn==1){
			nMoneyToEarn=40000;
		}else if(comboMoneyToEarn==2){
			nMoneyToEarn=50000;
		}else if(comboMoneyToEarn==3){
			nMoneyToEarn=75000;
		}else if(comboMoneyToEarn==4){
			nMoneyToEarn=100000;
		}
	...
	event Timer5(){
		...
		if(GetMissionTime()>10*60*20){
			for(i=0;i<15;++i){
				...
				if((rPlayer!=null) && rPlayer.IsAlive()){
					if(
						(rPlayer.GetNumberOfBuildings(buildingNormal)<=4)
						&&
						((rPlayer.GetNumberOfBuildings()-rPlayer.GetNumberOfBuildings(buildingNormal))<=2) && (rPlayer.GetNumberOfUnits()<=6)
					){
						for(j=0;j<15;++j){
						...
						for(j=0;j<15;++j){
							rPlayer2=GetPlayer(j);
							if(j!=i && rPlayer2!=null && rPlayer2.IsAlive() && rPlayer2.IsCommandEnabled(commandBuildingSpyPlayer0+i)){
	...
	command Combo1(int nMode) button comboMoney{
		comboMoney=nMode;
		if(comboMoneyToEarn<comboMoney){
			comboMoneyToEarn=comboMoney;
		}
	}
