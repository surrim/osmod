...
mission "translateGameTypeDieHard4"{
		...
		for(i=0;i<15;++i){
			...
			if(rPlayer!=null){
				...
				if(comboUnitsLimit==0){
					...
					rPlayer.SetMilitaryUnitsLimit(100000);
				}
		...
		for(i=0;i<15;++i){
			...
			if(rPlayer!=null){
				if(rPlayer.IsAlive()){
					...
					}else if(comboWinCondition==1){
						iCountBuilding=rPlayer.GetNumberOfBuildings();
					}else if(comboWinCondition==2){
						iCountBuilding=
					...
					}else if(comboWinCondition==3){
						iCountBuilding=
							rPlayer.GetNumberOfBuildings(buildingPowerPlant)
							+rPlayer.GetNumberOfBuildings(buildingSolarPower)
							+rPlayer.GetNumberOfBuildings(buildingEnergyBattery);
					}else if(comboWinCondition==4){
						iCountBuilding=
							rPlayer.GetNumberOfBuildings(buildingBase)
							+rPlayer.GetNumberOfBuildings(buildingFactory)
							+rPlayer.GetNumberOfBuildings(buildingWaterBase)
	...
	event Timer1(){
		...
		for(i=0;i<15;++i){
			rPlayer=GetPlayer(i);
			if(rPlayer!=null && rPlayer.GetMoney()<100000){
