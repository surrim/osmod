...
mission "translateGameTypeCTFMP"{
	...
	enum comboCTFType{
		...
		multi:"translateScriptAvailableResearches"
	...
	//sprawdza czy rPlayer ma flage w swojej bazie
		...
		if((flagEx!=null) && flagEx.IsLive() && !flagEx.IsTransported()
			&&
			(Distance
			 (flagEx.GetLocationX(), flagEx.GetLocationY(),
			  rPlayer.GetStartingPointX(),
			  rPlayer.GetStartingPointY())<=m_nD)
	...
	function int HaveAllFlagsCapturedByAllies(player rPlayer){
		...
		for (j=0; j<15; j=j+1){
			rAlly=GetPlayer(j);
			if((rAlly!=null) && rAlly.IsAlive()
				&& ((rAlly==rPlayer) || rPlayer.IsAlly(rAlly))){
			...
			if((rEnemy!=null) && (rEnemy!=rPlayer)
				...
				for (j=0; j<15; j=j+1){
					...
					if((rAlly!=null) && rAlly.IsAlive()
						...
						if((Distance
							 (flagExEnemy.GetLocationX(),
							  flagExEnemy.GetLocationY(),
							  flagExAlly.GetLocationX(),
							  flagExAlly.GetLocationY())<=m_nD)
							&& (flagExEnemy.GetLocationZ() ==
								flagExAlly.GetLocationZ())){
							bCapturedEnemy=true;
						}
					}
				}
	...
	state Initialize{
		...
		if(comboResearchTime==1)
			SetTimeDivider(2);
		if(comboResearchTime==2)
			SetTimeDivider(4);
		if(comboResearchTime==3)
			SetTimeDivider(8);
		...
		for(i=0;i<15;++i){
			...
			if(rPlayer!=null){
				...
				if(rPlayer.GetRace()==raceUCS)
					flagEx =
						rPlayer.CreateUnitEx(rPlayer.GetStartingPointX(),
											 rPlayer.GetStartingPointY(),
											 0, "AI\\CivilEmpty",
											 "UCSUCTF", null, null, null,
											 null);
				if(rPlayer.GetRace()==raceED)
					flagEx =
						rPlayer.CreateUnitEx(rPlayer.GetStartingPointX(),
											 rPlayer.GetStartingPointY(),
											 0, "AI\\CivilEmpty", "EDUCTF",
											 null, null, null, null);
				if(rPlayer.GetRace()==raceLC)
					flagEx =
						rPlayer.CreateUnitEx(rPlayer.GetStartingPointX(),
											 rPlayer.GetStartingPointY(),
											 0, "AI\\CivilEmpty", "LCUCTF",
											 null, null, null, null);
				...
				CreateArtefact("NEASPECIAL3",
							   rPlayer.GetStartingPointX()+1,
							   rPlayer.GetStartingPointY(), 0, 0,
							   artefactSpecialAIOther, i);
				CreateArtefact("NEASPECIAL3",
							   rPlayer.GetStartingPointX()+1,
							   rPlayer.GetStartingPointY(), 0, 0,
							   artefactSpecialAIOther, i);
				CreateArtefact("NEASPECIAL3", rPlayer.GetStartingPointX(),
							   rPlayer.GetStartingPointY()+1, 0, 0,
							   artefactSpecialAIOther, i);
				CreateArtefact("NEASPECIAL3", rPlayer.GetStartingPointX(),
							   rPlayer.GetStartingPointY()+1, 0, 0,
							   artefactSpecialAIOther, i);
		...
		for(i=0;i<15;++i){
			...
			if((rPlayer!=null) && (rPlayer.GetScriptData(0)==1)){
				...
				if((flagEx==null) || !flagEx.IsLive()){
					...
					if(rPlayer.GetRace()==raceUCS)
						flagEx =
							rPlayer.CreateUnitEx(flagX, flagY, flagZ,
												 "AI\\CivilEmpty",
												 "UCSUCTF", null, null,
												 null, null);
					if(rPlayer.GetRace()==raceED)
	...
	event Timer3(){
		...
		if(comboCTFType==0){	//captured flag defeats
							...
							if((j!=i) && (rEnemy!=null)
								...
								if((flagExEnemy!=null)
									...
									if(HaveFlagInBase(rEnemy)
										...
										rPlayer.Defeat();
										KillArea(rPlayer.GetIFF(), GetRight()/2, GetBottom()/2, 0, 256);	//tym samym zabijamy tez flage
										KillArea(rPlayer.GetIFF(),
												 GetRight()/2,
												 GetBottom()/2, 1, 256);
										break;
									}
					...
					if((rPlayer!=null) && rPlayer.IsAlive()){
						...
						for (j=0; j<15; j=j+1){
							...
							if((j==i) || (rPlayer2!=null)
								&& rPlayer2.IsAlive()
								&& rPlayer.IsAlly(rPlayer2)){
								if(!HaveAllFlagsCapturedByAllies
									(rPlayer2)){
				...
				if(rLastPlayer!=null){
					for(i=0;i<15;++i){
						...
						if((rPlayer!=null) && (rPlayer!=rLastPlayer)){
							...
							rPlayer.Defeat();
							KillArea(rPlayer.GetIFF(), GetRight()/2, GetBottom()/2, 0, 256);	//tym samym zabijamy tez flage
							KillArea(rPlayer.GetIFF(), GetRight()/2, GetBottom()/2, 1, 256);
						}
					}
					rLastPlayer.Victory();
				}
	...
	event Timer4(){
		...
		for(i=0;i<15;++i){
			rPlayer=GetPlayer(i);
			if(rPlayer!=null && rPlayer.GetMaxDistance()<255){
	...
	event Timer5(){
		...
		if(GetMissionTime()>10*60*20){
			for(i=0;i<15;++i){
				rPlayer=GetPlayer(i);
				if(rPlayer!=null && rPlayer.IsAlive()){
					if(rPlayer.GetNumberOfBuildings(buildingNormal)<=4 && (rPlayer.GetNumberOfBuildings()-rPlayer.GetNumberOfBuildings(buildingNormal))<=2 && rPlayer.GetNumberOfUnits()<=6){
						for(j=0;j<15;++j){
							rPlayer2=GetPlayer(j);
