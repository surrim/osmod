...
mission "translateGameTypeArena"{
	...
	enum comboTechLevel{
		...
		multi:"translateGameMenuTechLevel"
	...
	function int CreateStartingUnits(player rPlayer, player rPlayer2){
		...
		if(comboTechLevel==0){
			if(rPlayer.GetRace()==raceED){
				...
				rPlayer.CreateUnitEx(x, y, 0, null, "EDUMT3", "EDWSL1",
									 null, null, null, 0);
				if(comboStartingUnits>0)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "EDUST3",
										 "EDWCA2", null, null, null);
				if(comboStartingUnits>1)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "EDUMW3",
										 "EDWSR3", null, null, null, 0);
			}
			if(rPlayer.GetRace()==raceLC){
				rPlayer.CreateUnitEx(x, y, 0, null, "LCUMO2", "LCWSL1",
									 null, null, null, 0);
				if(comboStartingUnits>0)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "LCUMO3",
										 "LCWSR3", null, null, null);
				if(comboStartingUnits>1)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "LCUMO2",
										 "LCWSS2", null, null, null, 0);
			}
			if(rPlayer.GetRace()==raceUCS){
				rPlayer.CreateUnitEx(x, y, 0, null, "UCSUML3", "UCSWSSP2",
									 null, null, null, 2);
				if(comboStartingUnits>0)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "UCSUSL3",
										 "UCSWTSR3", null, null, null);
				if(comboStartingUnits>1)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "UCSUML3",
										 "UCSWSMR1", null, null, null, 2);
			}
		}
		if(comboTechLevel==1){
			if(rPlayer.GetRace()==raceED){
				rPlayer.CreateUnitEx(x, y, 0, null, "EDUHT3", "EDWMR3",
									 null, null, null, 1);
				if(comboStartingUnits>0)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "EDUHW2",
										 "EDWHL2", null, "EDWSL2", null,
										 2);
				if(comboStartingUnits>1)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "EDUHT3",
										 "EDWHC2", null, "EDWSR3", null,
										 1);
			}
			if(rPlayer.GetRace()==raceLC){
				rPlayer.CreateUnitEx(x, y, 0, null, "LCUCR1", "LCWMR3",
									 null, null, null, 2);
				if(comboStartingUnits>0)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "LCUCR1",
										 "LCWHL1", null, "LCWSL1", null,
										 2);
				if(comboStartingUnits>1)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "LCUCR1",
										 "LCWHS2", null, null, null, 2);
			}
			if(rPlayer.GetRace()==raceUCS){
				rPlayer.CreateUnitEx(x, y, 0, null, "UCSUHL3", "UCSWBMR3",
									 null, null, null, 2);
				if(comboStartingUnits>0)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "UCSUHL3",
										 "UCSWBHP3", null, "UCSWSSP2",
										 null, 2);
				if(comboStartingUnits>1)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "UCSUHL3",
										 "UCSWBSR3", null, "UCSWSSR3",
										 null, 2);
			}
		}
		if(comboTechLevel==2){
			if(rPlayer.GetRace()==raceED){
				rPlayer.CreateUnitEx(x, y, 0, null, "EDUHW2", "EDWHL3",
									 null, "EDWSL3", null, 2);
				if(comboStartingUnits>0)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "EDUBT2",
										 "EDWMR3", "EDWMR3", null, null,
										 1);
				if(comboStartingUnits>1)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "EDUHT3",
										 "EDWHC2", null, "EDWSR3", null,
			...
			if(rPlayer.GetRace()==raceLC){
				rPlayer.CreateUnitEx(x, y, 0, null, "LCUCR3", "LCWHL2",
									 null, "LCWSL2", null, 2);
				if(comboStartingUnits>0)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "LCUCU3",
										 "LCWMR3", "LCWMR3", null, null,
										 1);
				if(comboStartingUnits>1)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "LCUCU3",
										 "LCWHL2", "LCWHL2", "LCWSS2",
										 "LCWSS2", 1);
			}
			if(rPlayer.GetRace()==raceUCS){
				rPlayer.CreateUnitEx(x, y, 0, null, "UCSUBL2", "UCSWBHP3",
									 "UCSWSSP2", "UCSWSSP2", null, 2);
				if(comboStartingUnits>0)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "UCSUBL2",
										 "UCSWBMR3", "UCSWSSR3", null,
										 null, 2);
				if(comboStartingUnits>1)
					rPlayer.CreateUnitEx(x+1, y, 0, null, "UCSUBL2",
										 "UCSWBHP3", "UCSWSSP2",
	...
	event Timer2(){
		...
		rPlayer=null;
		for (i=nPlayerInConsole+1; i<15 && rPlayer==null;
			 i=i+1){
			rPlayer=GetPlayer(i);
			if(rPlayer!=null)
				nPlayerInConsole=i;
		...
		if(rPlayer!=null){
			...
			for(i=0;i<15;++i){
				...
				if(rPlayer2!=null
					&& (rPlayer.GetScriptData(0) <
						rPlayer2.GetScriptData(0)))
					k=k+1;
			}
		...
		for(i=0;i<15;++i){
			...
			if(rPlayer!=null && rPlayer.IsAlive()){
				...
				if(!rPlayer.GetScriptData(1)){
					...
					if(rPlayer2==null)
						rPlayer2=rPlayer;
					rPlayer.LookAt(rPlayer2.GetStartingPointX(),
								   rPlayer2.GetStartingPointY(), -1, -1,
								   -1, 0);
